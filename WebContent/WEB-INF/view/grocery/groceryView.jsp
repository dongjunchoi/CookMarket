...
$(function(){
	...
	$("#btnUpdate").on("click", function(){
		var form = $("#groceryForm").get(0);
		form.action = "<%=request.getContextPath()%>/grocery/groceryUpdateForm.do";
	...
	$("#btnDelete").on("click", function(){
		var form = $("#groceryForm").get(0);
		form.action = "<%=request.getContextPath()%>/grocery/groceryDelete.do";
		form.submit();  // form의 데이터를 서버로 전송
	});
...
<form id="groceryForm" method="post" action="">
	<input type="hidden" name="grocery_cod" id="grocery_cod" value="<%=groceryVo.getGrocery_cod() %>">
  <table class="table">
 <thead>
	<tr>
		<th>식료품코드</th>
		<th><%=groceryVo.getGrocery_cod() %></th>
	</tr>
	    </thead>
	<tr>
		<th>식료품명</th>
		<th><%=groceryVo.getG_name() %></th>
	</tr>
	<tr>
		<th>가격</th>
		<th><%=groceryVo.getG_price() %></th>
	</tr>
	<tr>
		<th>판매자코드</th>
		<th><%=groceryVo.getSeller_cod() %></th>
	</tr>
	    </thead>
	<tr>
