...
$(function(){
	...
	$('#cfbtn').on('click',function(){
		...
		if(mail== $('#Certificationbtn_1').val()){
			cfcheck = $('#cfcheck1').html("인증성공").css("color","green");
		}else{
			$('#cfcheck1').html("인증실패").css("color","red");
		}
	})
	...
	$('#ch').on('click',function(){
		...
		if(a="인증성공"){
			window.close();
			window.open("confirm2.jsp","","width=500, height=200");
		}
	})
...
<body>
	<div class="form-group">
		<label class="font-weight-bold text-white" for="Certificationbtn_1">인증번호확인</label>
		<div>
			<input style="width: 300px; float: left;" type="text" class="form-control" id="Certificationbtn_1" name="Certificationbtn_1" placeholder="ex) kls1asd12"> 
				<input type="button" class="form-control" id="cfbtn" name="cfbtn" value="인증확인"> <span id="cfcheck1"></span><br>
			<br>
		</div>
	</div>
	<input type="button" id="ch" value="확인">
