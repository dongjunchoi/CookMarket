...
//uri_mapping.properties파일에 설정된 URI에 맞는 Action객체를 구해와서 인스턴스를 생성해서 반환하는 역할을 수행하는 클래스
public class URIActionMapper {
	//uri_mapping.properties 파일의 내용을 읽어와 URI요청은 키값으로 Action객체명은 value값으로 저장될 Map객체 생성 
	private static Map<String, String> actionMap = new HashMap<>();
	
	//static초기화 블럭
	static {
		//properties파일을 읽어와 Bundle객체를 생성한다.
		ResourceBundle rb = ResourceBundle.getBundle("middle.web.uri_mapping");
	...
	//매개값으로 주어진 uri에 맞는 Action객체를 인스턴스화 해서 반환하는 메서드
