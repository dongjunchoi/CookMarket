...
public class AES256Util {
	...
	private static final String key = "a1b2c3d4e5f6g7h8"; //암호화 키 값 (16글자 이상)
	...
	public AES256Util() throws UnsupportedEncodingException {
		...
		//앞에 잇는 데이터의 앞에 부터 len만큼 복사해서 keybyte배열의 0번째 부터 붙여넣어라
	...
	public String encrypt(String str) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException {
		// - Cipher 생성 및 초기화 하기
		//  "AES/CBC/PKCS5Padding" ==> "알고리즘/암호화 모드/패딩"
		// CBC : Cipher Block Chaining Mode 
		//			==> 동일한 평문 블록과 암호문 블록 쌍이 발생하지 않도록 이전 단계의 암복화한 결과가 현 단계에 영향을 주는 운영 모드를 말한다.
		//				블록 암호화 운영모드 중 보안성이 가장 높은 암호화 방법으로 가장 많이 사용한다.
		//				암호화가 병렬적으로 처리되는 것이 아니라 순차적으로 수행되어야 한다는 단점이 있다.
		
		// 패딩 : Padding ==> 마지막 블록이 블록의 길이와 항상 딱 맞아 떨어지지 않을 수 있기 때문에  부족한 길이만큼 '0'으로 채우거나 임의의 비트들로 채워넣는 것을 의미한다.
		...
		//		   WARP_MODE(암호키를 캡슐화), UNWARP_MODE(암호키 캡슐화 해제)
	...
	//str  ==> 복호화할 암호화된 문자열
	//반환값  ==> 복호화된 문자열 
	public String descrypt(String str) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException{
		Cipher c = Cipher.getInstance("AES/CBC/PKCS5Padding");
